...
module.exports = class updateDP {
    ...
    async parse(idcheck, isblind, lastelement, path, element) {
        try {
            ...
            objectKeys.forEach(async (key) => {
                ...
                if (element[key] !== null && typeof element[key] === "object") {
                    this.parse(idcheck, isblind, lastelement[key], path + "." + key, element[key]);
                } else {
                    if (key === "productname") {
                        if (element[key] === "FRITZ!DECT 500") {
                            if (element.simpleonoff.state === "0" || element.simpleonoff.state === "") {
                                onoff = false;
                            } else {
                                onoff = true;
                            }
                            if (element.simpleonoff.state.toString() !== lastelement.simpleonoff.state.toString()) {
                                this.adapter.setState(path + ".colorcontrol.alexaonoff", onoff, true);
                            }
                        }
                    }
