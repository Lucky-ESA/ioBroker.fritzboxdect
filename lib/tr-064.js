...
class TR064 extends EventEmitter {
    ...
    async start() {
        ...
        if (commandURL != null && typeof commandURL === "string" && commandURL.indexOf("serviceList") != -1) {
            ...
            await this.parser
                ...
                .then(result => {
                    this.adapter.log.debug(`${this.config.ip} - tr64desc: ${JSON.stringify(result)}`);
                    this.services = this.services.concat(this.objectToPaths(result));
                    return true;
                })
                .catch(err => {
                    this.adapter.log.warn(`${this.config.ip} - Parseerror: ${err}`);
                    return false;
                });
            ...
            if (commandURL != null && typeof commandURL === "string" && commandURL.indexOf("serviceList") != -1) {
                ...
                await this.parser
                    .parseStringPromise(commandURL)
                    .then(result => {
                        this.adapter.log.debug(`${this.config.ip} - igddesc: ${JSON.stringify(result)}`);
                        this.services = this.services.concat(this.objectToPaths(result));
                        return true;
                    })
                    .catch(err => {
            ...
            if (this.config.tr_interval > 0) {
                this.adapter.log.info(
                    `${this.config.ip} - Start TR-064 interval with ${this.config.tr_interval} Minute(s)`,
                );
    ...
    async createWlanStates(ghz, ghz_dp) {
        ...
        common = {
            name: {
                en: `Wifi ${ghz} GHz MAC Address`,
                de: `WLAN ${ghz} GHz MAC-Adresse`,
                ru: `MAC-адрес Wi-Fi ${ghz} ГГц`,
                pt: `Endereço MAC do Wi-Fi de ${ghz} GHz`,
                nl: `Wifi ${ghz} GHz MAC-adres`,
                fr: `Adresse MAC Wi-Fi ${ghz} GHz`,
                it: `Indirizzo MAC Wi-Fi ${ghz} GHz`,
                es: `Dirección MAC de Wifi de ${ghz} GHz`,
                pl: `Adres MAC Wi-Fi ${ghz} GHz`,
                uk: `MAC-адреса Wi-Fi ${ghz} ГГц`,
                "zh-cn": `WiFi ${ghz} GHz MAC 地址`,
            },
            type: "string",
            role: "state",
            desc: `Set new Wifi ${ghz} GHz name`,
        ...
        await this.adapter.createDataPoint(`${this.config.dp}.TR_064.States.wlan${ghz_dp}Mac`, common, "state", null);
        common = {
            name: {
                en: `Wifi ${ghz} QRCode`,
                de: `Wifi ${ghz} QRCode`,
    ...
    async updateEnergy() {
        ...
        if (ecoStat && ecoStat.data) {
            if (ecoStat.data.fritzos && ecoStat.data.fritzos.energy == "0") {
                this.adapter.log.warn(`${this.config.ip} - Disabled Energy Monitoring!`);
                this.missingEnergy = 11;
                await this.adapter.delObjectAsync(`${this.config.dp}.TR_064.States.Energy`, { recursive: true });
            }
            if (this.firststart && this.missingEnergy < 10) {
                await this.adapter.createStateEnergyTR064(this.config);
            ...
            ++this.missingEnergy;
            this.adapter.log.warn(`${this.config.ip} - Error EcoState: ${ecoStat}`);
        }
        ...
        if (energy && energy.data && energy.data.drain && Array.isArray(energy.data.drain)) {
            ...
            this.adapter.log.debug(`${this.config.ip} - in progress!!!`);
        } else if (energy && energy.message && energy.message.includes("timeout")) {
            this.adapter.log.debug(`${this.config.ip} - timeout of 5000ms exceeded`);
        } else if (
            energy &&
            energy.data &&
            energy.data.fritzos &&
            energy.data.fritzos.Productname == "FRITZ!Repeater 1700"
        ) {
            this.adapter.log.debug(`${this.config.ip} - Error energy: ${JSON.stringify(energy)}`);
        } else {
            this.adapter.log.warn(`${this.config.ip} - Error energy: ${JSON.stringify(energy)}`);
        }
    }
    ...
    async updateTR064Byte() {
        ...
        await this.parser
            ...
            .then(async result => {
                this.adapter.log.debug(`${this.config.ip} - RESULTMONITOR: ${JSON.stringify(result)}`);
                if (
                    result &&
                    result["s:envelope"] &&
        ...
        await this.parser
            .parseStringPromise(data)
            .then(async result => {
                this.adapter.log.debug(`${this.config.ip} - RESULTCOMMON: ${JSON.stringify(result)}`);
                if (
                    result &&
                    result["s:envelope"] &&
                    result["s:envelope"]["s:body"] &&
    ...
    async updateTR064Wlan() {
        ...
        const dp_mac = {
            1: `${this.config.dp}.TR_064.States.wlan24Mac`,
            2: `${this.config.dp}.TR_064.States.wlan50Mac`,
            3: `${this.config.dp}.TR_064.States.wlanguestname`,
            4: "",
            5: "",
        };
        ...
        if (this.wlanCount > 3) {
            ...
            dp_mac["4"] = dp_mac["3"];
            dp_mac["3"] = `${this.config.dp}.TR_064.States.wlan60Mac`;
            if (this.wlanCount > 4) {
                dp["5"] = dp["4"];
                ...
                dp_mac["5"] = dp_mac["4"];
                dp_mac["4"] = `${this.config.dp}.TR_064.States.wlan70Mac`;
            }
        }
        this.adapter.log.debug(`${this.config.ip} - Count WLAN ${count}`);
        for (let i = 1; i < count; i++) {
            let wpa = "";
            let pw = "";
            let ssid = "";
            const wlan_resp = await this.requests(`${wlan.link}${i}`, `${wlan.service}${i}`, `${wlan.action}`, "");
            this.adapter.log.debug(`${this.config.ip} - WLAN: ${wlan_resp}`);
            if (wlan_resp != null && typeof wlan_resp === "string" && wlan_resp.indexOf("NewEnable") != -1) {
                ...
                const wlan_g_mac = wlan_resp.match("<NewBSSID>(.*?)</NewBSSID>");
                if (wlan_g_mac != null && wlan_g_mac[1] != null) {
                    if (!this.checkvalue[dp_mac[i]] || this.checkvalue[dp_mac[i]] != wlan_g_mac[1]) {
                        this.checkvalue[dp_mac[i]] = wlan_g_mac[1];
                        ssid = wlan_g_mac[1];
                        await this.adapter.setState(dp_mac[i], { val: wlan_g_mac[1], ack: true });
                    }
                }
                const wlan_secure = await this.requests(
                    `${wlan.link}${i}`,
    ...
    async loadRights() {
        ...
        this.adapter.log.debug(`${this.config.ip} - X_AVM-DE_GetCurrentUser: ${data}`);
        if (data != null && typeof data === "string" && data.indexOf("X_AVM-DE_GetCurrentUser") != -1) {
            data = entities.decodeHTML(data);
            await this.parser
                .parseStringPromise(data)
                .then(async result => {
                    this.adapter.log.debug(`${this.config.ip} - RESULTSecurity: ${JSON.stringify(result)}`);
    ...
    async updateTR064() {
        ...
        this.adapter.log.debug(`${this.config.ip} - RESULTDEVICE: ${data}`);
        let state = "";
        let loginfo = "";
        if (data != null && typeof data === "string" && data.indexOf("GetInfoResponse") != -1) {
            loginfo = await this.parser
                .parseStringPromise(data)
                .then(async result => {
                    this.adapter.log.debug(`${this.config.ip} - RESULTDEVICE: ${JSON.stringify(result)}`);
                    if (
                        result &&
                        result["s:envelope"] &&
                        result["s:envelope"]["s:body"] &&
        ...
        this.adapter.log.debug(`${this.config.ip} - GetSecurityPort: ${data}`);
        ...
        this.adapter.log.debug(`${this.config.ip} - RESULTINFO: ${data}`);
        if (data != null && typeof data === "string" && data.indexOf("GetInfoResponse") != -1) {
            await this.parser
                .parseStringPromise(data)
                .then(async result => {
                    this.adapter.log.debug(`${this.config.ip} - RESULTINFO: ${JSON.stringify(result)}`);
                    if (
                        result &&
                        result["s:envelope"] &&
                        result["s:envelope"]["s:body"] &&
                        ...
                        const val = result["s:envelope"]["s:body"]["u:getinforesponse"];
                        this.adapter.log.debug(`${this.config.ip} - RESULTINFO: ${JSON.stringify(val)}`);
        ...
        this.adapter.log.debug(`${this.config.ip} - RESULTINFOIPv6: ${data}`);
            ...
            await this.parser
                ...
                .then(async result => {
                    ...
                    if (
                        ...
                        this.adapter.log.debug(`${this.config.ip} - RESULTINFOIPv6: ${JSON.stringify(val)}`);
        ...
        this.adapter.log.debug(`${this.config.ip} - RESULTINFOIPv6Pre: ${data}`);
            ...
            await this.parser
                ...
                .then(async result => {
                    ...
                    if (
                        ...
                        this.adapter.log.debug(`${this.config.ip} - RESULTINFOIPv6Pre: ${JSON.stringify(val)}`);
    ...
    async requests(request, service, action, params) {
        ...
        return await exec(request).then(
            out => {
                this.adapter.log.debug(`${this.config.ip} - OUT: ${out.stdout} - ${out.stderr}`);
                try {
                    if (out.stdout) {
                        return out.stdout;
                    }
                    ...
                    this.adapter.log.debug(`${this.config.ip} - catch exec: ${JSON.stringify(e)}`);
                    return false;
                }
            },
            err => {
                this.adapter.log.debug(`${this.config.ip} - requests: ${JSON.stringify(err)}`);
                return false;
            },
        );
    }
    ...
    rigthsCheck(service, action) {
        ...
        if (action) {
            if (urn[service]) {
                ...
                this.adapter.log.warm(`${this.config.ip} - No action found for ${action}`);
                return false;
            }
            this.adapter.log.warm(`${this.config.ip} - No service found for ${service}`);
            return false;
        } else if (service) {
            if (urn[service]) {
                return urn[service];
