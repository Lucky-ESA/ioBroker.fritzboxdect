...
module.exports = class createDP {
    ...
    async parse(isblind, elements, path, element, options) {
        try {
            ...
            if (!this.alreadyCreatedOBjects[path]) {
                if (lastsplit.includes('DECT_grp')) icon = entries.pics["Group"];
                else if (elements.productname.split(" ")[1]) icon = entries.pics["D" + elements.productname.split(" ")[1]];
                else if (elements.productname === "HAN-FUN") icon = entries.pics["FUN"];
                else  icon = entries.pics["Unbekannt"];
                if (this.adapter.config.extendForeign && lastsplit.includes('DECT')) {
                    common = {
                        type: "channel",
                        common: {
                    ...
                    this.updatechannelobject(path, common);
                }
                await this.adapter
            ...
            if (lastsplit === "colorcontrol") {
                if (!this.alreadyCreatedOBjects[path + ".alexaonoff"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexaonoff", {
                            type: "state",
                            common: {
                                name: "Light On/Off",
                                role: "switch.light",
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexaonoff"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
            ...
            if (lastsplit === "hkr") {
                if (!this.alreadyCreatedOBjects[path + ".alexapower"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexapower", {
                            type: "state",
                            common: {
                                name: "HKR ON(Auto)/OFF",
                                role: "switch.power",
                                type: "boolean",
                                write: true,
                                read: true,
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexapower"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
                }
                if (!this.alreadyCreatedOBjects[path + ".alexamode"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexamode", {
                            type: "state",
                            common: {
                                name: "HKR Auto/OFF",
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexamode"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
                }
                if (!this.alreadyCreatedOBjects[path + ".alexaparty"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexaparty", {
                            type: "state",
                            common: {
                                name: "HKR True 8 Grad and false Auto",
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexaparty"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
            ...
            if (lastsplit === "levelcontrol" && isblind === 1) {
                if (this.adapter.config.extendForeign) {
                    this.delobject(path + ".levelalexa");
                }
                if (!this.alreadyCreatedOBjects[path + ".alexalevel"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexalevel", {
                            type: "state",
                            common: {
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexalevel"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
                }
                if (!this.alreadyCreatedOBjects[path + ".alexavalue"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexavalue", {
                            type: "state",
                            common: {
                                name: "0=close 100=open",
                                role: "value.blind",
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexavalue"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
                }
                if (!this.alreadyCreatedOBjects[path + ".alexastop"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexastop", {
                            type: "state",
                            common: {
                                name: "Blind Stop",
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexastop"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
                }
                if (!this.alreadyCreatedOBjects[path + ".alexaopen"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexaopen", {
                            type: "state",
                            common: {
                                name: "Blind Open",
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexaopen"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
                }
                if (!this.alreadyCreatedOBjects[path + ".alexaclose"]) {
                    await this.adapter
                        .setObjectNotExistsAsync(path + ".alexaclose", {
                            type: "state",
                            common: {
                                name: "Blind Close",
                        ...
                        .then(() => {
                            this.alreadyCreatedOBjects[path + ".alexaclose"] = true;
                        })
                        .catch((error) => {
                            this.adapter.log.error(error);
                        });
            ...
            objectKeys.forEach(async (key) => {
                ...
                if (element[key] !== null && typeof element[key] === "object") {
                    ...
                    if (!this.alreadyCreatedOBjects[path + "." + key]) {
                        if (this.adapter.config.extendForeign) {
                            this.updateobject(path + "." + key, common);
                        }
                        await this.adapter
                    ...
                    if (key === "levelpercentage" && isblind === 1) {
                        if (this.alreadyCreatedOBjects[path + ".alexalevel"]) {
                            ...
                            this.adapter.setState(path + ".alexalevel", valtf, true);
                        }
                        if (this.alreadyCreatedOBjects[path + ".alexavalue"]) {
                            valtf = 100 - element[key];
                            this.adapter.setState(path + ".alexavalue", valtf, true);
                        }
                    }
    ...
    updatechannelobject(path, com) {
        this.adapter.getForeignObject(this.adapter.namespace + "." + path, (err, obj) => {
            if (obj) {
                this.adapter.setObject(path, com);
            }
        });
    }

    updateobject(path, com) {
        this.adapter.getForeignObject(this.adapter.namespace + "." + path, (err, obj) => {
            if (obj) {
                obj = {
                    type: "state",
                ...
                this.adapter.setObject(this.adapter.namespace + "." + path, obj);
            }
        });
    }
    ...
    delobject(path) {
        this.adapter.getForeignObject(this.adapter.namespace + "." + path, (err, obj) => {
            if (obj) {
            this.adapter.log.warn("del: " + path);
                this.adapter.delForeignObject(this.adapter.namespace + "." + path);
            }
        });
    }
    ...
    addstate(element, st, blind) {
        ...
        const commons = { 
            ...
            simple: {"0": "false", "1": "true", "2": "toggle"},
    ...
    getRole(element, write, arr, st, blind) {
        ...
        if (st === "levelcontrol" && element === "levelpercentage" && blind === 1) element = "levelpercentageblind";
