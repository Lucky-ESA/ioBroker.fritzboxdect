    ...
    async parse(isblind, elements, path, element, options) {
        try {
            ...
            if (typeof element === "string" || typeof element === "number") {
                ...
                tf = this.getRole(lastsplit, options.write, "typ", st, isblind);
                if (!this.alreadyCreatedOBjects[path]) {
                    common = {
                        name: this.getRole(lastsplit, options.write, "name", st, isblind),
                        role: this.getRole(lastsplit, options.write, "role", st, isblind),
                        type: element !== null ? this.getRole(lastsplit, options.write, "typ", st, isblind) : "mixed",
                        write: this.getRole(lastsplit, options.write, "write", st, isblind),
                        read: true,
                    };
                    if (temp.includes(lastsplit) || lastsplit === "celsius") {
                        common.min = "-30";
                        common.max = 255;
                        common.unit = "°C";
                    }
                    if (grad.includes(lastsplit)) {
                        common.min = 0;
                        common.max = 359;
                        common.unit = "°";
                    }
                    if (proz.includes(lastsplit)) {
                    ...
                    if (lastsplit === "voltage") common.unit = "V";
                    if (lastsplit === "energy") common.unit = "Wh";
                    if (lastsplit === "power") common.unit = "W";
                    ...
                    if (lastsplit === "levelpercentage" && isblind === 1) {
                       ...
                       if (!this.alreadyCreatedOBjects[path + ".levelalexa"]) {
                        ...
                        element = 100 - element;
                        this.adapter.setState(path + ".levelalexa", element, true);
                    }
                }
                if (tf === "boolean") {
                    tf = (element === 0) ? false : true;
                } else if (tf === "number" && element === null) {
                    tf = 0;
                } else {
                    tf = element;
                }
                tf = (lastsplit === "celsius" && tf.toString().length > 2) ? tf / 10 : tf;
                tf = (temp.includes(lastsplit)) ? tf / 2 : tf;
                if (lastsplit === "voltage" && valtf > 0) tf = tf * 0.001;
                if (lastsplit === "power" && valtf > 0) tf = tf * 0.01;
                if (lastsplit === "tsoll") {
                    let units = null;
                    if (tf === 126.5) {
                        units = "";
                    ...
                    this.adapter.setObject(path, obj);
                }
                this.adapter.setState(path, tf, true);
                tf = "";
                return;
            }
